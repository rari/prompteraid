...
export default class AppController {
  ...
  init() {
    ...
    // Show copy button tip on first visit
    this.showCopyButtonTip();
  }
  ...
  /**
   * Create and show the copy button tip overlay
  ...
  createCopyButtonTip(copyButton) {
    ...
    tipOverlay.innerHTML = `
      <div class="copy-tip-content">
        <div class="copy-tip-header">
          <i class="far fa-clipboard copy-tip-icon"></i>
          <span class="copy-tip-title">5. Copy & Use</span>
          <button class="copy-tip-close" aria-label="Close tip">
            <i class="fas fa-times"></i>
          </button>
        </div>
        <p class="copy-tip-text">Click the copy button to copy your generated prompt to the clipboard, then paste it into your AI art tool!</p>
      </div>
    `;
  ...
  /**
   * Position the tip overlay relative to the copy button
   * @param {HTMLElement} tipOverlay - The tip overlay element
   * @param {HTMLElement} copyButton - The copy button element
   */
  positionTipOverlay(tipOverlay, copyButton) {
    const buttonRect = copyButton.getBoundingClientRect();
    const tipRect = tipOverlay.getBoundingClientRect();
    
    // Position above and to the right of the button
    const top = buttonRect.top - tipRect.height - 10;
    const left = buttonRect.left + buttonRect.width / 2 - tipRect.width / 2;
    ...
    // Adjust if tip would go off the top
    if (finalTop < 10) {
      finalTop = buttonRect.bottom + 10;
    }
    
    // Adjust if tip would go off the left
    if (finalLeft < 10) {
      finalLeft = 10;
    }
    
    // Adjust if tip would go off the right
    if (finalLeft + tipRect.width > viewportWidth - 10) {
      finalLeft = viewportWidth - tipRect.width - 10;
    }
    
    tipOverlay.style.top = `${finalTop}px`;
    tipOverlay.style.left = `${finalLeft}px`;
  }

  /**
   * Hide the copy button tip
   * @param {HTMLElement} tipOverlay - The tip overlay element
   */
  hideCopyButtonTip(tipOverlay) {
    ...
    localStorage.setItem('prompteraid_copyTipShown', 'true');
  }
} 